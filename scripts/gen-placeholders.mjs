...
// LQIP blur-up (P11): a tiny base64 placeholder per asset, keyed by the public
// path the components reference, so next/image can `placeholder="blur"`. Regenerate
// this whenever the real images land (it is derived from whatever files exist here).
async function blur(rel) {
  const buf = await sharp(path.join(root, "public", rel)).resize(20).blur(1.5).webp({ quality: 40 }).toBuffer();
  return `data:image/webp;base64,${buf.toString("base64")}`;
}
const blurMap = {};
for (const p of projects) blurMap[`/images/projects/${p.file}`] = await blur(`images/projects/${p.file}`);
blurMap["/images/profile/jay.jpg"] = await blur("images/profile/jay.jpg");
writeFileSync(
  path.join(root, "lib/image-blur.ts"),
  `// AUTO-GENERATED by scripts/gen-placeholders.mjs — do not edit by hand.\n` +
    ...
    `export const imageBlur: Record<string, string> = ${JSON.stringify(blurMap, null, 2)};\n`,
);
console.log(`generated ${n} project covers + 1 profile placeholder + ${Object.keys(blurMap).length} blur placeholders`);
